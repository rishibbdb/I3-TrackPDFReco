...
# def c_multi_gupta_mpe_logprob_midpoint2_stable(x, log_mix_probs, a, b, n, sigma=2.0):
#     """
#     Q < 30
#     """
#     nmax = 10
...
def c_multi_gupta_mpe_logprob_midpoint2_stable(x, log_mix_probs, a, b, n, sigma=2.0):
    """
    Compute log-probability using fine grid numerical integration.
    
    Uses adaptive grid spacing for accurate integration:
    - Fine resolution near peak (Gaussian is steep)
    - Coarser resolution in tails (Gaussian is flat)
    
    Args:
        x: Time residual (delay_time) in nanoseconds
        log_mix_probs: (4,) Log-probabilities of mixture components
        a, b: (4,) Gamma distribution parameters
        n: Observed photon count
        sigma: Time resolution (Gaussian width, ns)
    
    Returns:
        log_prob: Log-likelihood of observation
    """
    ...
    # ========================================================================
    # FINE GRID CONFIGURATION: ~250 fixed points
    # ========================================================================
    # Multiple fine regions near peak, coarser far away
    
    x0 = eps
    x_m0 = 0.005
    xvals0 = jnp.linspace(x0, x_m0, 20 * int_scale)[:-1]
    
    x_m1 = 0.01
    xvals1 = jnp.linspace(x_m0, x_m1, 20 * int_scale)[:-1]
    
    x_m2 = 0.05
    xvals2 = jnp.linspace(x_m1, x_m2, 20 * int_scale)[:-1]
    
    x_m3 = 0.15
    xvals3 = jnp.linspace(x_m2, x_m3, 20 * int_scale)[:-1]
    
    x_m4 = 0.25
    xvals4 = jnp.linspace(x_m3, x_m4, 20 * int_scale)[:-1]
    
    x_m5 = 0.50
    xvals5 = jnp.linspace(x_m4, x_m5, 20 * int_scale)[:-1]
    
    x_m6 = 0.75
    xvals6 = jnp.linspace(x_m5, x_m6, 20 * int_scale)[:-1]
    
    x_m7 = 1.5
    xvals7 = jnp.linspace(x_m6, x_m7, 20 * int_scale)[:-1]
    
    x_m8 = 2.5
    xvals8 = jnp.linspace(x_m7, x_m8, 20 * int_scale)[:-1]
    
    x_m9 = 5.0
    xvals9 = jnp.linspace(x_m8, x_m9, 30 * int_scale)[:-1]
    
    x_m10 = 8.0
    xvals10 = jnp.linspace(x_m9, x_m10, 30 * int_scale)
    

    xmin = jnp.max(jnp.array([1.5 * eps, x - 10 * sigma * int_scale]))
    xmax = jnp.max(jnp.array([xmin + 1.5*eps, x + 10 * sigma * int_scale]))
    xvals_adaptive = jnp.linspace(xmin, xmax, 151 * int_scale)

    xvals = jnp.sort(jnp.concatenate([xvals0, xvals1, xvals2, xvals3, xvals4, 
                                      xvals5, xvals6, xvals7, xvals8, xvals9, 
                                      xvals10, xvals_adaptive]))
...
###Rishi
def c_multi_gupta_mpe_logprob_midpoint2_stable_fast(x, log_mix_probs, a, b, n, 
                                                     sigma=3.0,
                                                     precomputed_grid_data=None):
    """
    Fast version that reuses precomputed grid data across calls.
    
    If precomputed_grid_data is provided, uses cached grids.
    Otherwise computes fresh (for single calls).
    """
    eps = 1.e-6
    int_scale = 1
    
    if precomputed_grid_data is None:
        # Fallback: compute grid fresh (slower, but works standalone)
        x0 = eps
        x_m0 = 0.01
        xvals0 = jnp.linspace(x0, x_m0, 10 * int_scale)[:-1]
        x_m1 = 0.05
        ...
        # Use precomputed fixed grid
        xvals = precomputed_grid_data['xvals']
        dx = precomputed_grid_data['dx']
        # Recompute Gaussian part only (depends on x and sigma)
    
    # Gaussian PDF (depends on x and sigma)
    log_n_pdf = norm_logpdf(xvals, loc=x, scale=sigma)
    
    # Physics part (depends on a, b, log_mix_probs, n)
    a_e = jnp.expand_dims(a, axis=-1)
    b_e = jnp.expand_dims(b, axis=-1)
    log_mix_probs_e = jnp.expand_dims(log_mix_probs, axis=-1)
    xvals_e = jnp.expand_dims(xvals, axis=0)
    
    # These could also be cached if a,b don't change
...
# Vectorized version
c_multi_gupta_mpe_logprob_midpoint2_stable_fast_v = jax.vmap(
    c_multi_gupta_mpe_logprob_midpoint2_stable_fast, 
    (0, 0, 0, 0, 0, None, None), 
    0
)
 
 
def precompute_fixed_grid(sigma_ref=3.0, eps=1.e-6, int_scale=1):
    """
    Precompute a fixed integration grid.
    Use for all x values within a reasonable range.
    
    Returns dict with 'xvals' and 'dx'
    """
    x0 = eps
    x_m0 = 0.01
    xvals0 = jnp.linspace(x0, x_m0, 10 * int_scale)[:-1]
    ...
    # Fixed range around reasonable delay times
    xmin = 1.5 * eps
    xmax = 50.0  # Adjust based on your typical delay times
    xvals_x = jnp.linspace(xmin, xmax, 501 * int_scale)
