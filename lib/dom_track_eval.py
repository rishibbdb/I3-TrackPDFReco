...
def get_eval_network_doms_and_track_wtilt(eval_network_v_fn, dtype=jnp.float64, gupta=False, n_comp=3):
    """
    network eval function (vectorized across doms)
    """
    ...
    @jax.jit
    def eval_network_doms_and_track_wtilt(dom_pos, track_vertex, track_dir):
        """
        track_direction: (zenith, azimuth) in radians
        track_vertex: (x, y, z)
        dom_pos: 2D array (n_doms X 3) where columns are x,y,z of dom location
    ...
    return eval_network_doms_and_track_wtilt
