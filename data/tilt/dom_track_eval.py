...
from lib.geo import cherenkov_cylinder_coordinates_w_rho_v, cherenkov_cylinder_coordinates_w_rho2_v, cherenkov_cylinder_coordinates_w_rho_v_icetilt
...
def get_eval_network_doms_and_track_w_icetilt(eval_network_v_fn, dtype=jnp.float64):
    """
    network eval function (vectorized across doms)
    """
    ...
    def eval_network_doms_and_track(dom_pos, track_vertex, track_dir):
        ...
        geo_time, closest_approach_dist, closest_approach_z, closest_approach_rho = cherenkov_cylinder_coordinates_w_rho_v_icetilt(dom_pos,track_vertex,track_dir_xyz)

        # x_np = jax.device_get(dom_pos)
        # y_np = jax.device_get(geo_time)
        # # print("JAXNP",x_np, y_np)
        # print("A",dom_pos, track_vertex, track_dir_xyz)
        # print("B",geo_time, closest_approach_dist)
...
def get_eval_network_doms_and_track_w_charge_w_icetilt(eval_network_v_fn, eval_charge_network_v_fn, dtype=jnp.float64):
    """
    network eval function (vectorized across doms)
    """
    ...
    def eval_network_doms_and_track(dom_pos, track_vertex, track_dir):
        ...
        geo_time, closest_approach_dist, closest_approach_z, closest_approach_rho = \
            cherenkov_cylinder_coordinates_w_rho_v_icetilt(dom_pos,
                                         track_vertex,
                                         track_dir_xyz)
        print(dom_pos, track_vertex, track_dir_xyz)
        print(geo_time, closest_approach_dist)
