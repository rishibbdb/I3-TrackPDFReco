...
parser.add_argument("--file-index-start", type=int,
                  default=10000,
                  dest="FILE_INDEX_START",
                  help="start index of range of files to be converted (included)")

parser.add_argument("--file-index-end", type=int,
                  default=20000,
                  dest="FILE_INDEX_END",
                  help="end index of range of files to be converted (excluded)")

parser.add_argument("--outdir-ftr", type=str,
                  default="/home/storage2/hans/i3files/21217/",
                  dest="OUTDIR",
                  help="directory where to write output feather files")

parser.add_argument("--outdir-tf", type=str,
                  default="/home/storage2/hans/i3files/21217/",
                  dest="OUTDIR_TF",
                  help="directory where to write output tensorflow files")
...
def framestats(frame):
    ...
    pass_muon_energy = True  ##### Keep all events regardless of muon energy at detector
    ...
    if not is_CC_interaction:
        print("Not a CC interaction. Skip!")
...
print(f"Found {len(i3files)} files matching pattern {file_pattern}")
pulse_frames = []
meta_frames = []
total_event_count = 0
for i, i3file in enumerate(i3files):
    ...
    # min_muon_energy_at_detector = 1000   # These cuts are not enforced anymore
